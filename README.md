...
`PORT` is supplied automatically by Render and should not be manually set
unless you have a specific deployment reason.

## Vercel

Set the Vercel project's **Root Directory** to `dashboard`.
...
`GHOSTEA_API_KEY` must exactly match Render's `DASHBOARD_API_KEY`.
`GHOSTEA_ADMIN_PASSWORD` belongs on Render, not Vercel.
